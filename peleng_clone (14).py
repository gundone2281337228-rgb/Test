...
# =============================================================================
# 1A2. ПЕРВЕРСИОННЫЕ КАРТЫ БАЙТОВ (etap G/H, findings #771-875)
# =============================================================================
# Три варианта прибора («Базовая» / «Железнодорожная» / «Рельсовая») имеют
# СОБСТВЕННЫЕ карты смещений полей в RAW-body. Эти карты выведены из
# дизассемблера HandlerA (fcn.00404758) / HandlerB (fcn.00403834) /
# HandlerC (fcn.004041d0) в 102_203dll.dll.
#
# TypeVar диапазоны определяют, какой handler обрабатывает запись:
#   • 150-151             → HandlerA  → Version 3 (рельсовая, ЖД-полотно)
#   • 360-771             → HandlerB  → Version 1 (базовая, толщиномер)
#   • 440-823             → HandlerC  → Version 2 (ЖД, колёсные пары)
#
# Каждый handler имеет ДВА маршрута: MAIN (метаданные, TYPEVAR @+0x5E) и
# SUB (тело записи, TYPEVAR @+0x29 — компактнее).
#
# Названия полей соответствуют именам в DLL (видны в .data сегменте
# @0x428700..0x4288A0) и в PelengPCtest.dal (RESULTS1/2/3 таблицы).

# Тип кодировки байта в RAW-body:
#   "lut11" / "lut7"   — PASSPORT_LUT (124-символьная подстановка, reverse-BCD)
#   "le16"             — fcn.0040381C: byte[0] | (byte[1] << 8)
#   "byte_int"         — IntToStr(byte) — STAGE/SECTION/PATH/PICKETLINE
#   "place_enum"       — 6-состояний enum (только V3 SUB), см. PLACE_ENUM
#   "defekt_bool"      — TypeVar.bit0 → "есть"/"нет" (читает record[+0xC])
#   "le16_div10"       — LE16 / 10 (CLOCK V1)
#   "date3"            — 3 байта (d, m, y-2000)
#   "time2"            — 2 байта (h, m)

# Версия 1 (Базовая) — MAIN запись (HandlerB, tcode 4/5/6).
BODY_LAYOUT_V1_MAIN: list[tuple[str, int, str]] = [
    # (field_name, body_offset, encoding)
    ("NUMOBJ",     0x11, "lut11"),       # 11 байт BCD/паспорт
    ("CODEDEF",    0x21, "lut7"),        # 7 байт BCD/код дефекта
    ("M",          0x35, "le16"),        # метры
    ("MM",         0x37, "le16"),        # миллиметры
    ("CLOCK",      0x39, "le16_div10"),  # часы (условно, /10)
    ("MAKETIME",   0x3C, "le16"),        # год изготовления (только TV 680-683)
    ("SMELTING",   0x41, "lut7"),        # плавка (только TV 680-683)
    ("TYPEVAR",    0x5E, "le16"),        # тип. вариант (LE16)
    # DEFEKT берётся из record_ptr[+0xC] bit0 (не из body)
]

# Версия 1 (Базовая) — SUB запись (HandlerB tcode 7..16/17..26).
BODY_LAYOUT_V1_SUB: list[tuple[str, int, str]] = [
    ("NUMOBJ",     0x11, "lut11"),
    ("CODEDEF",    0x21, "lut7"),
    ("TYPEVAR",    0x29, "le16"),        # SUB: typevar рано в записи
    ("M",          0x35, "le16"),
    ("MM",         0x37, "le16"),
    ("CLOCK",      0x39, "le16_div10"),  # SUB: безусловно
]

# Версия 2 (Железнодорожная) — MAIN запись (HandlerC).
BODY_LAYOUT_V2_MAIN: list[tuple[str, int, str]] = [
    ("NUMOBJ",     0x11, "lut11"),
    ("CODEDEF",    0x21, "lut7"),
    ("SMELTING",   0x35, "lut7"),        # V2: плавка @+0x35 (не +0x41!)
    ("INDMAKER",   0x3C, "le16"),        # завод-изготовитель
    ("MAKETIME",   0x3E, "le16"),        # V2: год @+0x3E (не +0x3C)
    ("TYPEVAR",    0x5E, "le16"),
]

# Версия 2 (Железнодорожная) — SUB запись.
BODY_LAYOUT_V2_SUB: list[tuple[str, int, str]] = [
    ("NUMOBJ",     0x11, "lut11"),
    ("CODEDEF",    0x21, "lut7"),
    ("TYPEVAR",    0x29, "le16"),
]

# Версия 3 (Рельсовая) — MAIN запись (HandlerA).
BODY_LAYOUT_V3_MAIN: list[tuple[str, int, str]] = [
    ("NUMOBJ",     0x11, "lut11"),
    ("CODEDEF",    0x21, "lut7"),
    ("STAGE",      0x36, "byte_int"),    # перегон (byte → IntToStr)
    ("KM",         0x38, "le16"),        # километр
    ("M",          0x3A, "le16"),
    ("MM",         0x3C, "le16"),
    ("PICKETLINE", 0x3E, "byte_int"),    # пикет
    ("PATH",       0x3F, "byte_int"),    # путь
    ("SECTION",    0x40, "byte_int"),    # звено
    ("CONDLENGTH", 0x45, "le16"),        # условная длина (мм)
    ("TYPEVAR",    0x5E, "le16"),
]

# Версия 3 (Рельсовая) — SUB запись (с PLACE).
BODY_LAYOUT_V3_SUB: list[tuple[str, int, str]] = [
    ("NUMOBJ",     0x11, "lut11"),
    ("CODEDEF",    0x21, "lut7"),
    ("TYPEVAR",    0x29, "le16"),
    ("PLACE",      0x35, "place_enum"),  # V3 SUB: место дефекта (6-state enum)
    ("STAGE",      0x36, "byte_int"),
    ("KM",         0x38, "le16"),
    ("M",          0x3A, "le16"),
    ("MM",         0x3C, "le16"),
    ("PICKETLINE", 0x3E, "byte_int"),
    ("PATH",       0x3F, "byte_int"),
    ("SECTION",    0x40, "byte_int"),
]

# 6-state PLACE enum (V3 SUB only, etap H §212).
# body[+0x35] → строка местоположения дефекта на железнодорожной инфраструктуре.
PLACE_ENUM: dict[int, str] = {
    0: "в пути",       # на основном пути (default)
    1: "на рсп",       # на расцепляющем стрелочном переводе
    2: "покм. зап.",   # покилометровая запись
    3: "стр. пер.",    # стрелочный перевод
    4: "стр. з-д",     # стрелочный завод
    5: " ",            # неизвестно (empty)
}

# Дефект (DEFEKT) — bool, читается из record_ptr[+0xC] bit0.
# Чётные TypeVar → "нет", нечётные → "есть".
DEFEKT_BOOL = {0: "нет", 1: "есть"}

# Условные TypeVar диапазоны (etap G §189 finding #771).
# Используется для определения версии handler-а по TypeVar.
TYPEVAR_RANGES_V1 = (
    (360, 361), (376, 377), (390, 391),
    (678, 681), (706, 707), (730, 731),
    (738, 739), (770, 771),
)
TYPEVAR_RANGES_V2 = (
    (440, 441), (476, 477), (642, 643), (822, 823),
)
TYPEVAR_RANGES_V3 = (
    (150, 151),
)
...
def typevar_to_version(typevar: int) -> int:
    """Определить версию прибора (1/2/3) по TypeVar.

    Args:
        typevar: TypeVar записи (record[+0xC] LE16, или body[+0x5E])
    Returns:
        1 (Базовая) / 2 (Железнодорожная) / 3 (Рельсовая) / 0 (unknown)
    """
    for lo, hi in TYPEVAR_RANGES_V3:
        if lo <= typevar <= hi:
            return 3
    for lo, hi in TYPEVAR_RANGES_V1:
        if lo <= typevar <= hi:
            return 1
    for lo, hi in TYPEVAR_RANGES_V2:
        if lo <= typevar <= hi:
            return 2
    return 0


def _read_le16(body: bytes, off: int) -> int | None:
    """Прочитать LE16 (fcn.0040381C).

    byte[0] | (byte[1] << 8) — реализация байт-в-байт по dis-asm.
    """
    if off + 2 > len(body):
        return None
...
def decode_body_versioned(body: bytes,
                          layout: list[tuple[str, int, str]],
                          typevar: int = 0,
                          record_ptr_c: int = 0) -> dict[str, object]:
    """Декодировать body согласно версионной карте смещений.

    Args:
        body: RAW body байты (после 16-байтного заголовка)
        layout: одна из BODY_LAYOUT_V{1,2,3}_{MAIN,SUB}
        typevar: TypeVar записи (для conditional полей V1)
        record_ptr_c: значение record[+0xC] (для DEFEKT bit0)

    Returns:
        Dict { field_name: decoded_value }
    """
    result: dict[str, object] = {}
    # V1-only conditional: MAKETIME/SMELTING extracted ONLY for TV 680-683.
    # Определяем V1 по смещению SMELTING @+0x41 (V1) vs @+0x35 (V2).
    is_v1 = any(name == "SMELTING" and off == 0x41 for name, off, _ in layout)

    for name, off, enc in layout:
        if off >= len(body):
            continue

        # MAKETIME / SMELTING — условные ТОЛЬКО для V1 (TV 680-683).
        # В V2 эти поля присутствуют всегда (offsets +0x35/+0x3E).
        if is_v1 and name in ("MAKETIME", "SMELTING") and typevar:
            if not (680 <= typevar <= 683):
                continue

        if enc == "lut11":
            if off + 11 <= len(body):
                result[name] = decode_passport_peleng(body[off:off + 11])
        elif enc == "lut7":
            if off + 7 <= len(body):
                result[name] = decode_passport_peleng(body[off:off + 7])
        elif enc == "le16":
            v = _read_le16(body, off)
            if v is not None:
                result[name] = v
        elif enc == "le16_div10":
            v = _read_le16(body, off)
            ...
            result[name] = body[off]
        elif enc == "place_enum":
            v = body[off]
            result[name] = PLACE_ENUM.get(v, " ")
        elif enc == "date3":
            if off + 3 <= len(body):
                d, m, y = body[off], body[off + 1], body[off + 2]
                result[name] = (d, m, 2000 + y)
        elif enc == "time2":
            if off + 2 <= len(body):
                h, mi = body[off], body[off + 1]
                if 0 <= h < 24 and 0 <= mi < 60:
                    result[name] = f"{h:02d}:{mi:02d}"

    # DEFEKT всегда из record_ptr[+0xC] bit0 (etap G §201)
    result["DEFEKT"] = DEFEKT_BOOL[record_ptr_c & 1]

    return result


def decode_body_v1_main(body: bytes, typevar: int = 0, record_c: int = 0) -> dict:
    """Декодер Version 1 (Базовая) MAIN-запись."""
    return decode_body_versioned(body, BODY_LAYOUT_V1_MAIN, typevar, record_c)


def decode_body_v1_sub(body: bytes, typevar: int = 0, record_c: int = 0) -> dict:
    """Декодер Version 1 (Базовая) SUB-запись (тело)."""
    return decode_body_versioned(body, BODY_LAYOUT_V1_SUB, typevar, record_c)


def decode_body_v2_main(body: bytes, typevar: int = 0, record_c: int = 0) -> dict:
    """Декодер Version 2 (Железнодорожная) MAIN-запись."""
    return decode_body_versioned(body, BODY_LAYOUT_V2_MAIN, typevar, record_c)


def decode_body_v2_sub(body: bytes, typevar: int = 0, record_c: int = 0) -> dict:
    """Декодер Version 2 (Железнодорожная) SUB-запись."""
    return decode_body_versioned(body, BODY_LAYOUT_V2_SUB, typevar, record_c)


def decode_body_v3_main(body: bytes, typevar: int = 0, record_c: int = 0) -> dict:
    """Декодер Version 3 (Рельсовая) MAIN-запись."""
    return decode_body_versioned(body, BODY_LAYOUT_V3_MAIN, typevar, record_c)


def decode_body_v3_sub(body: bytes, typevar: int = 0, record_c: int = 0) -> dict:
    """Декодер Version 3 (Рельсовая) SUB-запись (включая PLACE enum)."""
    return decode_body_versioned(body, BODY_LAYOUT_V3_SUB, typevar, record_c)


def decode_body_auto(body: bytes, record_c: int = 0,
                     prefer_sub: bool = False) -> tuple[int, dict[str, object]]:
    """Авто-определить версию по TypeVar (читаем @+0x5E или @+0x29) и декодировать.

    Args:
        body: RAW body
        record_c: record[+0xC] для DEFEKT
        prefer_sub: True если ожидаем SUB-запись (tcode 7..26)
    Returns:
        (version, decoded_dict)
    """
    # Сначала пробуем TYPEVAR @+0x5E (MAIN layout)
    tv_main = _read_le16(body, 0x5E) or 0
    tv_sub = _read_le16(body, 0x29) or 0

    if prefer_sub:
        tv = tv_sub or tv_main
    else:
        tv = tv_main or tv_sub

    ver = typevar_to_version(tv)
    if ver == 0:
        return 0, {}

    if prefer_sub:
        decoder = {1: decode_body_v1_sub, 2: decode_body_v2_sub, 3: decode_body_v3_sub}[ver]
    else:
        decoder = {1: decode_body_v1_main, 2: decode_body_v2_main, 3: decode_body_v3_main}[ver]
    return ver, decoder(body, tv, record_c)


# Русские подписи полей для GUI (соответствуют ENUM в PelengPCtest.dal).
...
def _encode_raw_body_versioned(layout: list[tuple[str, int, str]],
                                values: dict[str, object],
                                size: int = 0x80) -> bytes:
    """Закодировать RAW-body согласно версионной карте смещений (V1/V2/V3).

    Используется для генерации тестовых данных, эмулирующих реальные RAW-записи,
    которые DLL получает от прибора. Поля кодируются по их смещению и типу
    encoding (lut11/lut7/le16/byte_int/place_enum/le16_div10).
    """
    body = bytearray(size)
    for name, off, enc in layout:
        v = values.get(name)
        if v is None:
            continue
        if enc == "lut11":
            body[off:off + 11] = encode_passport_peleng(str(v), 11)
        elif enc == "lut7":
            body[off:off + 7] = encode_passport_peleng(str(v), 7)
        elif enc == "le16":
            iv = int(v) & 0xFFFF
            body[off:off + 2] = iv.to_bytes(2, "little")
        elif enc == "le16_div10":
            iv = (int(v) * 10) & 0xFFFF
            body[off:off + 2] = iv.to_bytes(2, "little")
        elif enc == "byte_int":
            ...
            # Принимаем индекс или строку
            if isinstance(v, str):
                for k, s in PLACE_ENUM.items():
                    if s == v:
                        body[off] = k
                        break
                else:
...
def _fake_v1_record(typevar: int = 390, seed: int = 0) -> TLVRecord:
    """Сгенерировать RAW V1 (Базовая) MAIN запись для демо."""
    values = {
        "NUMOBJ":   f"K-{100 + seed:03d}",
        "CODEDEF":  f"D{seed % 10}",
        "M":        40 + seed,
        "MM":       350,
        "CLOCK":    8,
        "MAKETIME": 2024,
        "SMELTING": f"{3145 + seed}",
        "TYPEVAR":  typevar,
    }
    body = _encode_raw_body_versioned(BODY_LAYOUT_V1_MAIN, values)
    return TLVRecord(tag=0x0010, length=4 + len(body), body=body)


def _fake_v2_record(typevar: int = 440, seed: int = 0) -> TLVRecord:
    """Сгенерировать RAW V2 (Железнодорожная) MAIN запись для демо."""
    values = {
        "NUMOBJ":   f"WA-{1000 + seed:04d}",
        "CODEDEF":  f"D{seed % 10}",
        "SMELTING": f"{4500 + seed}",
        "INDMAKER": 27,        # завод-изготовитель (LE16 код)
        "MAKETIME": 2023,
        "TYPEVAR":  typevar,
    }
    body = _encode_raw_body_versioned(BODY_LAYOUT_V2_MAIN, values)
    return TLVRecord(tag=0x0020, length=4 + len(body), body=body)


def _fake_v3_record(typevar: int = 150, seed: int = 0) -> TLVRecord:
    """Сгенерировать RAW V3 (Рельсовая) MAIN запись для демо."""
    values = {
        "NUMOBJ":     f"R-{500 + seed:03d}",
        "CODEDEF":    f"D{seed % 10}",
        "STAGE":      42 + (seed % 10),
        "KM":         1234 + seed,
        "M":          567,
        ...
        "PATH":       3,
        "SECTION":    12,
        "CONDLENGTH": 185 + seed * 5,
        "TYPEVAR":    typevar,
    }
    body = _encode_raw_body_versioned(BODY_LAYOUT_V3_MAIN, values)
    return TLVRecord(tag=0x0030, length=4 + len(body), body=body)


def make_fake_block_versioned(seed: int = 0) -> tuple[bytes, ProtocolHeader]:
    """Демо-блок с одной MAIN-записью каждой версии (V1/V2/V3) + Settings/Report.

    Полезен для проверки 3-таб GUI: после вставки в БД заполняются все 3
    вкладки «Базовая/Железнодорожная/Рельсовая».
    """
    records = [
        _fake_settings(),
        _fake_v1_record(typevar=390, seed=seed),
        _fake_v2_record(typevar=440, seed=seed),
        _fake_v3_record(typevar=150, seed=seed),
        _fake_ascan(seed),
        _fake_bscan(seed),
        _fake_report(),
    ]
    payload = encode_tlv(records)
...
def run_gui(argv: list[str], demo: bool = False) -> int:
    ...
    class MainWindow(QMainWindow):
        ...
        def _build_central(self):
            ...
            # ----- 3 версии прибора («Базовая» / «Железнодорожная» /
            # «Рельсовая») — отражают TypeVar dispatch в оригинальной DLL
            # (HandlerB / HandlerC / HandlerA соответственно). Каждая вкладка
            # показывает декодированные записи именно той версии и со своими
            # колонками (V1 имеет M/MM/CLOCK; V2 — SMELTING/INDMAKER/MAKETIME;
            # V3 — KM/M/MM/PLACE/STAGE/SECTION/PICKETLINE/PATH).
            self._version_tables: dict[int, QTableWidget] = {}
            self._version_layouts: dict[int, list[tuple[str, int, str]]] = {
                1: BODY_LAYOUT_V1_MAIN,
                2: BODY_LAYOUT_V2_MAIN,
                3: BODY_LAYOUT_V3_MAIN,
            }

            def _mk_ver_table(layout):
                # Колонки = ["№", "Дата", "Время", DEFEKT] + поля из layout.
                cols = ["№", "Дата", "Время", FIELD_LABELS_RU["DEFEKT"]]
                cols += [FIELD_LABELS_RU.get(n, n) for n, _, _ in layout]
                t = QTableWidget(self)
            ...
            self.tab_v1 = _mk_ver_table(BODY_LAYOUT_V1_MAIN)
            self.tab_v2 = _mk_ver_table(BODY_LAYOUT_V2_MAIN)
            self.tab_v3 = _mk_ver_table(BODY_LAYOUT_V3_MAIN)
            self._version_tables[1] = self.tab_v1
            self._version_tables[2] = self.tab_v2
            self._version_tables[3] = self.tab_v3

            self.version_subtabs = QTabWidget(self)
            self.version_subtabs.addTab(self._make_filter_panel(
                self.tab_v1, "Поиск Базовая: №/Дата/Объект/Код/Плавка…"),
                "Базовая (V1)")
            self.version_subtabs.addTab(self._make_filter_panel(
                self.tab_v2, "Поиск Железнодорожная: №/Дата/Плавка/Завод…"),
                "Железнодорожная (V2)")
            self.version_subtabs.addTab(self._make_filter_panel(
                self.tab_v3, "Поиск Рельсовая: №/Дата/КМ/Перегон/Звено…"),
                "Рельсовая (V3)")
            ...
            # Versioned device records (3 sub-tabs).
            self.tabs.addTab(self.version_subtabs, "Версии прибора")
        ...
        def _on_demo_block(self):
            ...
            #    Используем make_fake_block_versioned, чтобы заполнить все 3
            #    вкладки «Базовая/Железнодорожная/Рельсовая» одним блоком.
            seed = self._db.count() + 1
            try:
                blob, hdr = make_fake_block_versioned(seed=seed)
        ...
        def _on_about(self):
            QMessageBox.about(self, "О программе",
                ...
                "дефектоскопу «Пеленг» (производитель: Altek, "
                "<a href='http://www.altek.info'>www.altek.info</a>).</p>"
                "<p><b>Поддержка 3 версий прибора:</b></p>"
                "<ul>"
                "<li><b>Базовая (V1)</b> — толщиномер, "
                "TypeVar 360–771 (HandlerB)</li>"
                "<li><b>Железнодорожная (V2)</b> — колёсные пары/вагоны, "
                "TypeVar 440–823 (HandlerC)</li>"
                "<li><b>Рельсовая (V3)</b> — рельсовое полотно, "
                "TypeVar 150–151 (HandlerA)</li>"
                "</ul>"
                "<p><b>Протокол реверсирован</b> из оригинального ПО "
                "(Borland C++Builder, 2009): 875 находок задокументированы в "
                "<code>PELENG_REVERSE_ROUND5.md</code> "
                "(этапы A–H, поля byte-offsets, опкоды, формулы UZ→дБ/мм).</p>"
                "<p><b>Опкоды протокола:</b> 0x55 хэндшейк (4× 'U'), "
                "0x42 блок-чтение, 0x74 flash-дамп (128 KB/512 KB), "
                "0x9A снимок экрана (4293/5253 байт)</p>"
                "<p><b>Зависимости:</b> PyQt6, pyserial, openpyxl, "
                "fdb (опц. — импорт PelengPC.fdb Firebird Embedded 2.x).</p>")
        ...
        def _refresh_versions(self):
            """Заполнить 3 вкладки Базовая/Железнодорожная/Рельсовая декодированными
            записями. Каждый BlockZap → парсим TLV → каждая TLV-запись пытается
            быть декодирована как MAIN (TYPEVAR @+0x5E) → диспатч на V1/V2/V3.
            """
            # Очищаем все 3 таблицы
            buckets: dict[int, list[tuple[BlockRow, TLVRecord, dict]]] = {1: [], 2: [], 3: []}
            for row in self._db.all():
                try:
                    block = row.block
                    if not block or len(block) < HEADER_LEN + 2:
                        continue
                    tlvs = list(parse_tlv(block))
                except Exception:
                    continue
                for tlv in tlvs:
                    body = tlv.body
                    if len(body) < 0x60:
                        continue
                    # Читаем TYPEVAR @+0x5E (MAIN layout). record[+0xC] — заголовок TLV.
                    tv = _read_le16(body, 0x5E) or 0
                    ver = typevar_to_version(tv)
                    if ver == 0:
                        continue
                    decoder = {1: decode_body_v1_main,
                               2: decode_body_v2_main,
                               3: decode_body_v3_main}[ver]
                    # record_c байт для DEFEKT bit0 — это TypeVar.bit0
                    rc = tv & 1
                    fields = decoder(body, tv, rc)
                    buckets[ver].append((row, tlv, fields))

            for ver, items in buckets.items():
                tbl = self._version_tables[ver]
                layout = self._version_layouts[ver]
                tbl.setSortingEnabled(False)  # быстрее массовая загрузка
                tbl.setRowCount(len(items))
                col_keys = [n for n, _, _ in layout]
                for i, (row, tlv, fields) in enumerate(items):
                    hdr = row.header()
                    item0 = QTableWidgetItem(str(row.number))
                    # сохраняем номер BlockZap в UserRole для двойного клика
                    item0.setData(Qt.ItemDataRole.UserRole, int(row.number))
                    tbl.setItem(i, 0, item0)
                    tbl.setItem(i, 1, QTableWidgetItem(hdr.date_str()))
                    tbl.setItem(i, 2, QTableWidgetItem(hdr.time_str()))
                    tbl.setItem(i, 3, QTableWidgetItem(str(fields.get("DEFEKT", ""))))
                    for c, key in enumerate(col_keys, start=4):
                        v = fields.get(key, "")
                        if isinstance(v, str):
                            v = v.strip()
                        tbl.setItem(i, c, QTableWidgetItem(str(v)))
                tbl.setSortingEnabled(True)
                # Двойной клик по строке → выбрать BlockZap слева → А-развёртка
                if not getattr(tbl, "_dblclick_wired", False):
                    tbl.cellDoubleClicked.connect(
                        lambda r, c, t=tbl: self._on_version_row_open(t, r))
                    tbl._dblclick_wired = True

        def _on_version_row_open(self, tbl, row_idx):
            """Двойной клик по строке в таб «Базовая/ЖД/Рельсовая» — открывает
            этот BlockZap в левой таблице и переключается на А-развёртку."""
            ...
            if number is None:
                try:
                    ...
                    return
            # Найти строку с этим номером в основной таблице
            for r in range(self.table.rowCount()):
                cell = self.table.item(r, 0)
                if cell and cell.text() == str(number):
                    self.table.selectRow(r)
                    # переключиться на А-развёртку (вкладка №0)

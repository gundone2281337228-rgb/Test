...
## 63. Команда 0x9A — Скриншот LCD дисплея (ПОДТВЕРЖДЕНО)

**Источник:** `Получение_экрана.html` (UART-перехват 13.05.2026, прибор 4428)
...
```
TX: 0x9A (1 байт)
RX: ~4281 байт (полный bitmap экрана)
Латентность: ~46 мс до первого чанка, ~850 мс полный ответ
```

### 63.2 Формат экрана (ПОДТВЕРЖДЕНО рендерингом)

| Параметр | Значение |
|----------|----------|
| Ширина | **240 пикселей** |
| Байт/строка | **30** (240 / 8 = 30) |
| Высота | **~142 строки** |
| Бит/пиксель | **1** (ЧБ bitmap) |
| Кодирование | bit=1 → ЧЁРНЫЙ, bit=0 → БЕЛЫЙ |
| Размер данных | 30 × 142 = **4260 байт** (+21 байт overhead) |
| Бит-порядок | MSB first (старший бит = левый пиксель) |

### 63.3 Содержимое экрана (из рендеринга)

```
┌─────────────────────────────────────────────────────────────┐
│ ┌──── Текстовые поля (параметры) ────┐ ┌── A-scan ────────┐│
│ │ Настройка: ...                     │ │ ████████████████  ││
│ │ Развёртка: ...                     │ │ █              █  ││
│ │ Скорость: ...                      │ │ █   █          █  ││
│ │ Строб 1: ...                       │ │ █   █          █  ││
...
Экран разделён на:
- **Левая часть** (~60% ширины): текстовые параметры настройки (кириллический bitmap-шрифт)
- **Правая часть** (~40% ширины): A-scan развёртка в реальном времени (прямоугольная область с рамкой)
- **Нижняя строка**: статусная информация

### 63.4 Связь с B-scan форматом

Формат скриншота **ИДЕНТИЧЕН** формату B-scan из §16.3:

| Параметр | Скриншот (0x9A) | B-scan (§16.3) |
|----------|-----------------|----------------|
| Байт/строка | 30 | 30 |
| Бит/строка | 240 | 240 |
| Кодирование | 1 bit/px, MSB first | 1 bit/px, MSB first |
| Цвета | bit=1→BLACK | bit=1→BLACK |
| Строк | ~142 | 128 (базовая) / 160 (вагонная) |

**Вывод:** Прибор использует ЕДИНЫЙ формат bitmap для всех графических данных — и для скриншотов экрана, и для B-scan развёрток. Различается только количество строк.

### 63.5 Рендеринг в Python

```python
def render_screen_0x9A(data: bytes, width_bytes: int = 30) -> list[str]:
    """Рендерит скриншот LCD из ответа 0x9A.
    
    Args:
        data: raw bytes from 0x9A response (~4281 bytes)
        width_bytes: 30 (= 240 pixels)
    
    Returns:
        List of strings, each 240 chars ('█' or ' ')
    """
    rows = len(data) // width_bytes
    lines = []
    for r in range(rows):
        row_data = data[r * width_bytes : (r + 1) * width_bytes]
        line = ''
...
def save_screen_as_png(data: bytes, filename: str, width_bytes: int = 30):
    """Сохраняет скриншот LCD как PNG файл."""
    width_px = width_bytes * 8  # 240
    height_px = len(data) // width_bytes  # ~142
    
    # Создаём 1-bit bitmap → PIL Image
    from PIL import Image
    img = Image.new('1', (width_px, height_px), 1)  # white background
    
    for r in range(height_px):
        for col_byte in range(width_bytes):
            byte_val = data[r * width_bytes + col_byte]
            for bit in range(8):
                x = col_byte * 8 + (7 - bit)
...
### 63.6 Сравнение с flash_dump.bin

| Файл | Тип | Размер | Содержимое |
|------|-----|--------|-----------|
| flash_dump.bin (прибор 45) | Настройки | 4007 байт | DGS-параметры, НЕ скриншот |
| 0x9A response (прибор 4428) | **Скриншот** | 4281 байт | LCD bitmap 240×142 |

**ВАЖНОЕ УТОЧНЕНИЕ:** `flash_dump.bin` в репозитории — это блок настроек (из другого прибора, device #45, info=0x18). Ответ на 0x9A от прибора 4428 (info=0x01) — это **скриншот LCD**.

Вероятно, поведение команды 0x9A **зависит от info_byte прибора**:
- info=0x18 (extended+bscan) → ответ = блок настроек (DGS-параметры)
- info=0x01 (minimal) → ответ = скриншот LCD

Или оба прибора отвечают скриншотом, но flash_dump.bin был обрезан/модифицирован при сохранении.

---

## 64. Сводка находок (UART-лог отчётов + скриншот) — 164-176

| # | Находка | Статус |
|---|---------|--------|
| 164 | 0x9A response = BITMAP СКРИНШОТ LCD дефектоскопа (240×142, 30 B/row) | 🔒 Железобетонно |
| 165 | Формат скриншота ИДЕНТИЧЕН формату B-scan (1 bit/px, MSB first) | 🔒 Железобетонно |
| 166 | Размер скриншота: ~4260 байт данных + ~21 байт overhead | 🔒 Железобетонно |
| 167 | Экран: левая часть=текст настроек, правая=A-scan, низ=статус | 🔒 Железобетонно |
| 168 | Запись 85 байт: [0-1]=echo, [2-3]=0x56 0x00 MARKER, [4-5]=TypeVar | 🔒 Железобетонно |
| 169 | Дата в записи: [7]=DAY [8]=MONTH [9]=YEAR-2000 [10]=HOUR [11]=MIN | 🔒 Железобетонно |
| 170 | peleng_reader.py ОШИБКА: [7]=month [8]=day → НЕВЕРНО! Реально [7]=day [8]=month | 🔒 Железобетонно |
| 171 | Маркер "V" (0x56 0x00) на позиции [2-3] в КАЖДОЙ записи | 🔒 Железобетонно |
| 172 | BCD объект: [17-22] обратный порядок, терминатор 0x0A, 6 цифр | 🔒 Железобетонно |
| 173 | BCD плавка: [53-57] обратный порядок, 5 цифр | 🔒 Железобетонно |
| 174 | Год изготовления: LE16 @ [62-63] (1976-2025 подтверждено) | 🔒 Железобетонно |
| 175 | Сторона: [66] (1=Лев, 2=Прав, 3=Обе), Шейка: [67] (0=Откр, 1=С кольц) | 🔒 Железобетонно |

...
## 30. РЕВЕРС РАУНД 5 — Реальный протокол УД2-102 (Вагонная версия)

**Дата:** 2026-05-13
**Прибор:** УД2-102 Вагонная версия, COM3 @ 19200 бод
**Подтверждено:** реальными данными с прибора

---

### 30.1 Полный протокол общения

```
КОМАНДА         ФОРМАТ                          ОТВЕТ
─────────────────────────────────────────────────────────────────
0x55 Handshake  [0x55]                         16 байт header + каталог (LE16[])
0x42 BlockReq   [0x42][5мс][addr_lo][5мс][addr_hi][5мс]  85 байт запись / 2 байта пустой
0x9A FlashDump  [0x9A]                         5253 байт (вагонная) / 4293 (базовая)
0x74 Status     [0x74]                         переменный размер
```

### 30.2 Handshake (0x55) — КАТАЛОГ АДРЕСОВ

Ответ прибора на handshake содержит **каталог всех заполненных адресов** в памяти:
- Первые 16 байт = header
- Остальные байты = массив LE16 адресов

Формула из оригинала: `numElements = (received - 0x10) / 2 - 1`

Каталог содержит ~290 адресов (для прибора с 30 измерениями). Включает:
- cat 1 (1000-1999): Настройки (~19 шт)
- cat 4-6 (4000-6999): Протоколы (~8 шт)
- cat 10-19 (10000-19999): Результаты/B-scan (~130 шт) ← ОСНОВНЫЕ ДАННЫЕ
- cat 20-29 (20000-29999): ShortProt/A-scan (~50 шт)

**ВАЖНО:** Многие адреса из каталога = ПУСТЫЕ блоки (0xFD FF). Каталог показывает ВСЕ адреса которые когда-либо использовались, не только текущие.

### 30.3 Block Request (0x42) — Чтение записи

Формат запроса:
```
PC → Прибор:  [0x42] [5мс] [addr & 0xFF] [5мс] [(addr >> 8) & 0xFF] [5мс]
Прибор → PC:  85 байт (запись) или 2 байта (пустой блок)
```

Пустой блок = `0xFD 0xFF` или `0xFF 0xFF`

**КРИТИЧНО:** Перед каждым запросом нужен `reset_input_buffer()` — иначе остатки от предыдущего handshake попадают в ответ.

### 30.4 Формат записи (85 байт) — ПОДТВЕРЖДЕНО ЭТАЛОНОМ

Эталон: addr=10101, дата 01.02.2026 09:27, номер 471845, плавка 53397, завод 18, год 2005, сторона правая

```
Offset  Size  Поле                    Подтверждено  Значение (пример)
──────  ────  ──────────────────────  ────────────  ──────────────────
0x00    2     echo addr (LE16)        ✓             10101
0x02    2     device_id (LE16)        ✓             86 (0x0056)
0x04    2     typevar (LE16)          ✓             21 (0x0015)
0x06    1     DAY                     ✓✓✓           1 (01.02.2026)
0x07    1     MONTH                   ✓✓✓           2 (февраль)
0x08    1     YEAR - 2000             ✓✓✓           26 (2026)
0x09    1     HOUR                    ✓✓✓           9
0x0A    1     MINUTE                  ✓✓✓           27
0x0B    1     (reserved)              ?             0
0x0C    2     defect_flags (LE16)     частично      0x002C (=44)
0x0E    2     ??? (LE16)              ?             0x0009
0x10    1     side/channel            частично      1 (правая?)
0x11    11    passport_primary (LUT)  частично      LUT-encoded (471845?)
0x1C    11    reserved/zeros          -             все 0x00
0x27    1     passport_secondary      ?             0x06
0x28    2     typevar2 (LE16)         ?             834 (0x0342)
0x2A    10    reserved/zeros          -             все 0x00
0x34    5     measurement values      меняется      различные
0x39    2     padding                 ✓             0x0A 0x0A
0x3B    1     probe_angle             стабильно     18 (0x12)
0x3C    1     (reserved)              -             0x00
0x3D    6     extra_params            меняется      различные
0x43    17    reserved/zeros          -             все 0x00
0x54    1     checksum/last           меняется      CRC?
```

### 30.5 Формат даты — ЖЕЛЕЗОБЕТОННО ПОДТВЕРЖДЕНО

```c
// Offset 0x06: [day][month][year-2000][hour][minute]
// Из FUN_004023b0 в DLL:
day   = raw[0x06];
month = raw[0x07];
year  = raw[0x08] + 2000;
hour  = raw[0x09];
...
Подтверждение: записи 10101-10106 делались подряд, минута растёт 27→28→29.

### 30.6 Passport LUT — формат

```c
// FUN_00402708: reverse byte order + LUT lookup
// Байты читаются от ПОСЛЕДНЕГО к ПЕРВОМУ
// idx 0 = NUL (пропуск), idx 1-9 = цифры '1'-'9', idx 10 = '0'
// idx 0x0A = пробел (в zapis2 LUT)
```

Пример: bytes `[05, 04, 08, 01, 07, 04, 0a, 0a, 0a, 0a, 0a]`
- reverse: 0a(skip) 0a(skip) 0a(skip) 0a(skip) 0a(skip) 04→'4' 07→'7' 01→'1' 08→'8' 04→'4' 05→'5'
- result: "471845" ← НОМЕР ОБЪЕКТА ✓

### 30.7 Скорость чтения

При 19200 бод:
- Handshake: ~0.5с (580 байт)
- Один блок (85 байт): ~50мс
- Пустой блок (2 байта): ~20мс (но idle timeout добавляет 500мс!)
- 150 адресов × 50мс = ~7.5с минимум
- Реально: ~10-15с (из-за пустых блоков с таймаутом)

### 30.8 Адресация блоков — структура

```
Адрес = BASE + INDEX
BASE = (addr / 100) * 100  → группа (10100, 10200, ...)
INDEX = addr % 100          → номер записи в группе (0-99)

Категория = addr / 1000:
  1     Настройки прибора
  4-6   Протоколы контроля
  10-19 Результаты (отчёты о контроле)
  20-29 A-развёртки (waveform data)
```

### 30.9 Нерешённые вопросы (TODO)

1. **Offset 0x0E-0x0F** — что это? В эталоне = 9, но год объекта = 2005. Не совпадает ни с чем.
2. **Плавка (53397)** — где в записи? Offset 0x11-0x1B = passport_primary (номер 471845). А плавка?
3. **Завод (18)** — где?
4. **Год выпуска объекта (2005)** — где? Возможно в offset 0x3D-0x3E (LE16 `d5 07` = 0x07D5 = 2005!)
5. **Обод (>50мм)** — где?
6. **Размер записи**: 10101 вернул 113 байт (первый запрос после handshake имел мусор). Чистая запись = 85 байт.

### 30.10 Flash dump (0x9A) — формат

Flash dump прибора — это **RAW memory image** в проприетарном формате (НЕ TLV-поток PelengPC!).
- Первые 2 байта: 0x002D (45) — возможно количество записей
- Offset 0x02: текущая дата прибора (24.03.2025 13:30)
- Паттерн `09 02` повторяется с интервалом 28 байт (записи настроек?)
- Паттерн `fd fd fd` = разделитель/padding
- Формат непригоден для прямого парсинга без полного реверса RAM-layout прибора

**Вывод:** Для чтения записей использовать каталог (handshake) + блочные запросы (0x42), а не flash dump.

...
## 29. Определение режима измерения — `FUN_00411799`

```c
int GetMeasurementMode(byte* rawPacket) {
    // Байт [400] = 0x190 offset в буфере пакета
    byte flagByte = rawPacket[400];
    
    if ((flagByte & 1) == 0) {
        // Бит 0 не установлен
        if ((rawPacket[0x101] & 2) == 0) {
            return 0;   // Базовый режим (без расширений)
        }
        return 1;       // Режим с расширенным DGS
    }
    
    // Бит 0 установлен → специальный режим
    _DAT_004cb6e8 = 0x191;  // магическое число (401 dec)
    
    switch (flagByte & 6) {   // биты 1-2
        case 0: return 4;     // Двойной A-scan
        case 2: return 2;     // Стробированный
        case 4: return 5;     // Расширенный двойной
        case 6: return 3;     // Комбинированный
        default: return 0xFF; // Ошибка
    }
}
```

**Ключевые смещения:**
- `rawPacket[400]` (0x190) — байт флагов режима
- `rawPacket[0x101]` (257) — байт расширенных флагов

---

## 30. Именовательные callback'и TypeVar — `FUN_004118b0`, `FUN_0041192c`

### 30.1 FUN_004118b0 — Имя категории (по param_1/100)

| param_1 / 100 | DAT адрес | Предполагаемое имя |
|---------------|-----------|-------------------|
| 1, 2, 3, 8, 9 | 0x4d8fb4 | "Настройки" |
| 4 | 0x4d8fc4 | "Развёртка" |
| default | 0x4d8fd8 | "Результаты" |
| 50-55 (param_1=50..55) | 0x4d8fa4 | "Калибровка" |

### 30.2 FUN_0041192c — Подтип (по param_1/10)

| param_1 / 10 | DAT | Предполагаемый подтип |
|--------------|-----|----------------------|
| 5 | 0x4d8fdc | (подтип для 50-59) |
| 11 (0xB) | 0x4d8fec | (подтип 110-119) |
| 12 (0xC) | 0x4d8ffc | (подтип 120-129) |
| 20, 30 | 0x4d900c | (подтип 200-209, 300-309) |
| 41 (0x29) | 0x4d901c | (подтип 410-419) |
| 42 (0x2A) | 0x4d9028 | (подтип 420-429) |
| 81-84 (0x51-54) | 0x4d903c | (подтип 810-849) |
| 90 (0x5A) | 0x4d9044 | (подтип 900-909) |
| default | 0x4d9054 | (по умолчанию) |

### 30.3 FUN_00411a1a — Развёрнутое имя (по param_1)

| param_1 | DAT | Предполагаемый тип |
|---------|-----|-------------------|
| 50 (0x32) | 0x4d9094 | "Толщина" |
| 51 (0x33) | 0x4d90a8 | "Скорость" |
| 52 (0x34) | 0x4d90b0 | "Время пробега" |
| 53 (0x35) | 0x4d90c8 | "Расстояние" |
| 54 (0x36) | 0x4d90e0 | "Координата Х" |
| 55 (0x37) | 0x4d90fc | "Координата Y" |
| 810-819 | 0x4d9058 | "Ось колёсной пары" |
| 820-829 | 0x4d9068 | "Бандаж" |
| 830-839 | 0x4d9070 | "Обод" |
| 840-849 | 0x4d9078 | "Гребень" |

### 30.4 FUN_00411f11 — Имя канала (по param_1/100)

| param_1 / 100 | DAT | Предполагаемое |
|---------------|-----|----------------|
| 0 | 0x4d9280 | "Канал 1" |
| 1 | 0x4d9294 | "Канал 2" |
| 2 | 0x4d92a8 | "Канал 3" |
...
## 31. Вспомогательные функции проверки полей

### 31.1 FUN_00411e1d — HasGate (есть ли строб?)

```c
bool HasGate(byte* rawPacket) {
    byte val = ReadByte(rawPacket + 0x147);  // offset 327
    return val > 0x10;  // если > 16 → строб есть
}
```

### 31.2 FUN_00411e3e — HasGateAndMode

```c
bool HasGateAndMode(byte* rawPacket) {
    if (ReadByte(rawPacket + 0x147) > 0x10) {
        if (ReadByte(rawPacket + 0xAF) != 0) {
            return true;   // есть и строб, и режим
        }
    }
    return false;
}
...
### 31.3 FUN_00411e8a — GetDivisionMode

```c
int GetDivisionMode(byte* rawPacket) {
    short val = ReadShort(rawPacket + 0x7E);  // offset 126
    int mode = val / 100;
    if (mode < 5) return 1;  // ×1
    return 0;                 // ×0 (не используется?)
}
```

### 31.4 FUN_00411ebc — GetChannelType

```c
int GetChannelType(byte* rawPacket) {
    short val = ReadShort(rawPacket + 0x7E);
    int type = val / 100;
    if (type == 8 || type == 0) return 1;  // специальный канал
    return 0;                               // обычный
}
```
...
## 32. FUN_00411dee — Тип B-scan (3 варианта)

```c
char* GetBScanType(int param) {
    if (param == 0) return "A";       // DAT_004d9270: только A-scan
    if (param == 1) return "B";       // DAT_004d9274: только B-scan
    return "A+B";                     // DAT_004d927c: комбинированный
}
```

---

## 33. Ключевые смещения в сыром пакете (сводка)

| Offset (hex) | Offset (dec) | Размер | Назначение | Источник |
|---|---|---|---|---|
| 0x00 | 0 | 2 | devId (LE16) | всё |
| 0x07 | 7 | 3 | date [d,m,y-2000] | FUN_004023b0 |
| 0x0A | 10 | 2 | time [h,m] | FUN_00402504 |
| 0x0C | 12 | 1 | defect_flag | FUN_00405974 |
| 0x10 | 16 | 2 | blockAddr (LE16) | _SortBufData |
| 0x11 | 17 | 11 | passport_primary (LUT) | FUN_00405cf8 |
| 0x14 | 20 | 2 | TypeVar / tcode (LE16) | DAT_0051cb34 |
| 0x20 | 32 | 2 | sweep/category code | FUN_0040afd1 |
| 0x21 | 33 | 7 | passport_secondary (LUT) | FUN_00405b58 |
| 0x27 | 39 | 5 | date2 (BCD-формат?) | FUN_0040afd1 |
| 0x35 | 53 | 2 | thickness_mm (LE16) | FUN_004055ac |
| 0x37 | 55 | 2 | velocity_mps (LE16) | FUN_004056b4 |
| 0x38 | 56 | 2 | amplitude_dB (LE16) | FUN_00405528 |
| 0x39 | 57 | 2 | depth_mm (LE16) | FUN_00406204 |
| 0x3A | 58 | 2 | delay_us (LE16) | FUN_00405630 |
| 0x3C | 60 | 2 | gate_position (LE16) | FUN_00405738 |
| 0x3E | 62 | 2 | echo_count (LE16) | FUN_00406158 |
| 0x45 | 69 | 2 | zone_width (LE16) | FUN_00405c78 |
| 0x4B | 75 | 2 | some_flag (LE16) | FUN_0041183e |
| 0x5E | 94 | 2 | total_length (LE16) | FUN_00406318 |
| 0x7E | 126 | 2 | channel_code (LE16) | FUN_00411e8a |
| 0xAF | 175 | 1 | mode_flag | FUN_00411e3e |
| 0x101 | 257 | 1 | extended_flags | FUN_00411799 |
| 0x147 | 327 | 1 | gate_threshold | FUN_00411e1d |
| 0x190 | 400 | 1 | mode_byte (главный) | FUN_00411799 |

---

## 34. Сводка новых находок (раунд 7)

| # | Находка | Статус | Источник |
|---|---------|--------|----------|
| 93 | Режим измерения определяется по byte[400] (бит 0 + биты 1-2) → 6 вариантов | 🔒 Железобетонно | FUN_00411799 |
| 94 | 4 уровня именования: категория→подтип→полное имя→канал (param/100, /10, точное) | 🔒 Железобетонно | FUN_004118b0..FUN_00411f11 |
| 95 | Смещения TypeVar в пакете: +0x14 (LE16), +0x20 (LE16 category×1000) | 🔒 Железобетонно | FUN_0040afd1 |
| 96 | HasGate: byte[0x147] > 0x10 → строб активен | 🔒 Железобетонно | FUN_00411e1d |
| 97 | DivisionMode: ReadShort(+0x7E)/100 → ×1 если <5, иначе ×0 | 🔒 Железобетонно | FUN_00411e8a |
| 98 | ChannelType: ReadShort(+0x7E)/100 == 8 или 0 → специальный | 🔒 Железобетонно | FUN_00411ebc |
| 99 | BScanType: 3 варианта (0="A", 1="B", 2="A+B") | 🔒 Железобетонно | FUN_00411dee |
| 100 | Полная карта 12 смещений в пакете (0x4B, 0x7E, 0xAF, 0x101, 0x147, 0x190) | 🔒 Железобетонно | множество FUN |



---

## 35. Конкретные поля отчёта — инициализация массива (`FUN_0041375a`)

### 35.1 Полная реконструкция массива полей для вагонной версии

Функция `FUN_0041375a` инициализирует массив FieldDescriptor (stride 28 байт) по адресу `DAT_004ea2a8`:

| # | Offset в буфере | Тип | editName | visible_cb | Семантика |
|---|---|---|---|---|---|
| 0 | DAT_004c0064 (packed) | 0x30 | no_edit | — | **Дата измерения** (packed date) |
| 1 | 0x15D (349 dec) | 0x04 | no_edit | — | **Номер записи** (LE16 @ +349) |
| 2 | DAT_004c0060 (packed) | 0x31 | no_edit | — | **Амплитуда/порог** (dual-byte) |
| 3 | 0x0005005B (HI=0x50=80, LO=0x5B=91) | 0x25 | ifLength | — | **Толщина** (условное: показать если длина>0) |
| 4 | 0x0005005D (HI=0x50=80, LO=0x5D=93) | 0x25 | ifHight | — | **Амплитуда** (условное: показать если>порога) |
| 5 | 0x01470160 (HI=0x147=327, LO=0x160=352) | 0x42 | no_edit | FUN_00414088 | **Скорость звука** (Real48 с порогом 0x10/0x7E) |
| 6 | 0x01470160 | 0x43 | no_edit | FUN_00414088 | **Диаметр** = 2×√(Real48 @ +352) |
| 7..N | (0x276 нулевых записей) | — | — | — | (резерв / дополнительные поля) |
| N+1 | 0x2C (44 dec) | 0x41 | — | — | (подтип: 0x48=72, формат 0x41) |
| N+2 | 0x7E (126 dec) | 0x3E | defekt_edit | FUN_00413f6f | **Описание дефекта** (составное, 2 источника, РЕДАКТИРУЕМОЕ) |

### 35.2 Callback-и «Сторона» и «Шейка» — `FUN_00413602`, `FUN_00413631`

**FUN_00413602** — декодирует поле **«Сторона»** (тип 0x27):
```c
char* DecodeSide(int raw_value) {
    if (raw_value == 1) return "Левая";        // DAT_004e8968
    if (raw_value == 2) return "Правая";       // DAT_004e8978
    return "Не определена";                     // DAT_004e898c
}
```

**FUN_00413631** — декодирует поле **«Шейка»** или **«Тип контроля»** (тип 0x27, 7 вариантов):
```c
char* DecodeNeckType(int raw_value) {
    switch (raw_value) {
        case 0: return DAT_004e8990;   // "С кольцами"
        case 1: return DAT_004e8994;   // "Без колец"
        case 2: return DAT_004e8998;   // "С втулкой"
        case 3: return DAT_004e89a0;   // вариант 4
        case 4: return DAT_004e89a4;   // вариант 5
        case 5: return DAT_004e89a8;   // вариант 6
        default: return DAT_004e89ac;  // "—"
    }
}
```

**FUN_004136f7** — декодирует **«Тип объекта»** по category/100:
```c
char* DecodeObjectType(int category) {
    if (category / 100 == 1) return DAT_004e89b0;  // "Ось"
    if (category / 100 == 2) return DAT_004e89c0;  // "Колёсная пара"
    return "";                                       // DAT_0051c908
}
```

### 35.3 Именовательный callback TypeVar — `FUN_004139dc`

Возвращает строку-имя типового варианта по `param_1/100`:

| param_1/100 | DAT | Предполагаемое имя |
|---|---|---|
| 0 | 0x0051c920 | "" (пустая) |
| 1 | 0x004ee6e8 | "Толщиномер" |
| 2 | 0x004ee6f4 | "Ось" |
| 3 | 0x004ee700 | "Бандаж" |
| 4 | 0x004ee720 | "Колёсная пара" |
| 5 | 0x004ee730 | "Обод" |
| 6 | 0x004ee73c | "Гребень" |
| 7 | 0x004ee764 | "Наплавка" |
| 8 | 0x004ee780 | "Категория" |

### 35.4 Ключевые выводы по полям SHORTPROT/RESULTS

Из реверса `FUN_0041375a` + callback'ов **железобетонно подтверждено**:

1. **«Сторона»** — тип 0x27 + callback `FUN_00413602`: raw byte → "Левая"/"Правая"/"Не определена"
2. **«Шейка»** — тип 0x27 + callback `FUN_00413631`: raw byte → 7 вариантов (0..6)
3. **«Тип объекта»** — callback `FUN_004136f7`: category/100 → "Ось"/"Колёсная пара"/""
4. **«Дата»** — тип 0x30 (packed date из 20-bit decompose)
5. **«Толщина»** — тип 0x25, условное поле "ifLength" (показывается только если длина > 0)
6. **«Амплитуда»** — тип 0x25, условное поле "ifHight" (если превышает порог)
7. **«Скорость звука»** — тип 0x42 (Real48 с проверкой порога)
8. **«Диаметр»** — тип 0x43 (2×√Real48 — вычисляемое из площади)
9. **«Дефект»** — тип 0x3E, editName="defekt_edit" — **единственное редактируемое поле**!
10. **TypeVar имена** по callback `FUN_004139dc`: нумерация /100 (1=Толщиномер, 2=Ось, 3=Бандаж, 4=КП, 5=Обод, 6=Гребень, 7=Наплавка, 8=Категория)

### 35.5 Passport поля

Поля «Плавка», «Номер объекта», «Индекс завода» декодируются через:
- **Тип 0x28** = `FUN_0042409a(offset, 7)` — passport 7 байт (reverse LUT → строка)
- **Тип 0x2A** = `FUN_0042409a(offset, 6)` — passport 6 байт
- **Тип 0x39** = `FUN_0042409a(offset, 12)` — passport 12 байт
- **Тип 0x3B** = `FUN_0042409a(offset, 11)` — passport 11 байт

Конкретные offset'ы для каждого варианта конфигурации лежат в массивах `DAT_004c0720..DAT_004fbd18` (не могут быть извлечены из дизасма без прямого дампа `.data` секции PE).

### 35.6 Ключевые смещения полей в сыром пакете (подтверждённые)

| Raw offset | Тип | Поле | Источник |
|---|---|---|---|
| +0x07..+0x09 | date[3] | Дата (day,month,year-2000) | FUN_004023b0 |
| +0x0A..+0x0B | time[2] | Время (hours,minutes) | FUN_00402504 |
| +0x0C | flag | Дефект (0=нет) | FUN_00405974 |
| +0x11..+0x1B | passport[11] | Плавка / Номер объекта (LUT) | FUN_00405cf8 |
| +0x21..+0x27 | passport[7] | Индекс завода / Серийный (LUT) | FUN_00405b58 |
| +0x35 | LE16 | Толщина (мм×10?) | FUN_004055ac |
| +0x39 | LE16 | Глубина (мм) | FUN_00406204 |
| +0x7E | LE16 | Код канала/категории | FUN_00411e8a |
| +0xAF | byte | Флаг режима | FUN_00411e3e |
| +0x106 | byte | Флаг строба | FUN_00413696 |
| +0x137 | byte | Битовые флаги (бит 2) | FUN_00413696 |
| +0x147 | byte | Порог строба (>0x10 = активен) | FUN_00411e1d |
| +0x15D | LE16 | Номер записи | FUN_0041375a |
| +0x160 | Real48(6) | Скорость звука / параметр | FUN_0041375a |
| +0x190 | byte | Байт режима измерения | FUN_00411799 |

---

## 36. Сводка (раунд 8 — поля отчёта)

| # | Находка | Статус | Источник |
|---|---------|--------|----------|
| 101 | «Сторона»: callback FUN_00413602, 3 варианта (1=Левая, 2=Правая, else=?) | 🔒 Железобетонно | FUN_00413602 |
| 102 | «Шейка/тип»: callback FUN_00413631, 7 вариантов (0..6) | 🔒 Железобетонно | FUN_00413631 |
| 103 | «Тип объекта»: callback FUN_004136f7 по category/100 (1=Ось, 2=КП) | 🔒 Железобетонно | FUN_004136f7 |
| 104 | TypeVar именование: /100 → 8 категорий (Толщиномер..Категория) | 🔒 Железобетонно | FUN_004139dc |
| 105 | «Дефект» — единственное редактируемое поле ("defekt_edit", тип 0x3E) | 🔒 Железобетонно | FUN_0041375a |
| 106 | Массив полей начинается с DAT_004ea2a8, stride=28 байт, 7 int | 🔒 Железобетонно | FUN_0041375a |
| 107 | Поля Плавка/НомерОбъекта: через passport LUT (тип 0x28/0x2A/0x39/0x3B) | 🔒 Железобетонно | FUN_0042413f |
| 108 | Gate threshold > 0x10: строб активен; byte[0x106]&byte[0x137] бит 2 | 🔒 Железобетонно | FUN_00413696 |
| 109 | Скорость звука: Real48 @ offset 0x160 в буфере (352 dec) | 🔒 Железобетонно | FUN_0041375a тип 0x42 |
| 110 | Диаметр: 2×√(Real48) @ тот же offset — вычисляемое из площади | 🔒 Железобетонно | FUN_0041375a тип 0x43 |



---

## 37. HTML-генератор таблиц настроек — `FUN_0040c7d4`

### 37.1 Полная структура HTML-отчёта

```html
<!-- Для каждой секции полей: -->
<table border=0 align='center'>

  <!-- Тип 0 (заголовок/разделитель): -->
  <tr><td><b>LABEL_TEXT</b></td></tr>

  <!-- Обычные поля (тип != 0 и != 0x1C): -->
  <tr><td>FIELD_LABEL</td><td>DECODED_VALUE</td></tr>

  <!-- После half-point (середина секции): -->
</table></td><td><table border=0   <!-- переход на вторую колонку -->
...
### 37.2 Формат WordML для настроек — `FUN_0040d02b`

```xml
<w:body>
  <wx:sect>
    <!-- Для заголовков (type==0): -->
    <w:p>
      <w:pPr>
        <w:rPr>
          <w:u w:val="single"/>   <!-- подчёркивание! -->
        </w:rPr>
      </w:pPr>
    ...
    <!-- Для обычных полей: -->
    <w:p>
      <w:r><w:t>FIELD_LABEL: VALUE</w:t></w:r>
    </w:p>
...
### 37.3 Ключевые данные из `FUN_0040afd1` (рендерер настроек)

```c
// 1. Категория определяется по raw_buf[+0x20]:
uint category_code = LE16(DAT_0051cb58 + 0x20) & 0xFFFF;
int category = category_code / 1000;
// category==1 → "Настройки" (DAT_004b23d8)
// category==2 → "Результаты" (DAT_004b2408)

// 2. Множитель пути (ключевое!):
byte version = ReadByte(this->dgsParams + 0x6CC);
if (version == 2) {
    _DAT_00515ea0 = 2;   // вагонная: удвоенный путь
} else {
    _DAT_00515ea0 = 1;   // обычный
}

// 3. Цикл по полям (stride 0x0C = 12 байт):
// this+0x60 = количество полей
// this + field_idx * 0x0C + 0x78 = label (char*)
// this + field_idx * 0x0C + 0x7C = data_offset (int)
// this + field_idx * 0x0C + 0x80 = fieldType (int)

// 4. Секции разделяются типом 0x1C (сдвиг вправо +0x96=150px)
// Поля делятся пополам: после mid-point → переход на правую колонку
```

---

## 38. Исправление: тип 7 = **Шейка** (3 варианта), НЕ преобразователь

### 38.1 Из DGS-декодера `FUN_0040b7fd`, case 7 (строка ~11716):

```c
case 7:
    byte val = ReadByte(offset);
    if (val == 0) → DAT_004b24f4;   // "Открытая"
    if (val == 1) → DAT_004b24fc;   // "С кольцами"
    if (val == 2) → DAT_004b2504;   // "С буксой"
    // Ровно 3 варианта!
```

### 38.2 Тип 0x0E = **Тип преобразователя** (4 варианта):

```c
case 0x0E:
    byte val = ReadByte(offset);
    if (val == 0) → DAT_004b2534;   // "Круглый"
    if (val == 1) → DAT_004b2540;   // "Прямоугольный"
    if (val == 2) → DAT_004b2548;   // "Эллиптический"
    else          → DAT_004b2554;   // (неизвестный тип)
```

### 38.3 Тип 9 = **Boolean** (Вкл/Выкл):

```c
case 9:
    byte val = ReadByte(offset);
    if (val == 0) → DAT_004b24e4;   // "Выкл"
    else          → DAT_004b24ec;   // "Вкл"
```

---

## 39. Сводка находок (раунд 9)

| # | Находка | Статус | Источник |
|---|---------|--------|----------|
| 111 | **Тип 7 = Шейка** (0=Открытая, 1=С кольцами, 2=С буксой) — ровно 3 варианта! | 🔒 Железобетонно | FUN_0040b7fd case 7 |
| 112 | Тип 0x0E = Преобразователь (0=Круг, 1=Прямоуг, 2=Эллипс, 3=?) | 🔒 Железобетонно | FUN_0040b7fd case 0x0E |
| 113 | HTML-таблица: `<table border=0 align='center'>`, 2 колонки через mid-point | 🔒 Железобетонно | FUN_0040c7d4 |
| 114 | WordML настроек: `<w:cols w:num="2"/>` — всегда 2 колонки | 🔒 Железобетонно | FUN_0040d02b |
| 115 | Заголовки в WordML: `<w:u w:val="single"/>` (подчёркивание) | 🔒 Железобетонно | FUN_0040d02b |
| 116 | Категория из raw_buf[+0x20]: /1000 → 1="Настройки", 2="Результаты" | 🔒 Железобетонно | FUN_0040afd1 |
| 117 | Множитель пути: byte[DGS+0x6CC]==2 → ×2 (вагонная), else ×1 | 🔒 Железобетонно | FUN_0040afd1 |
| 118 | Stride DGS-полей: 0x0C (12 байт): +0x78=label, +0x7C=offset, +0x80=type | 🔒 Железобетонно | FUN_0040afd1 |
| 119 | Секция разделяется: type==0x1C → сдвиг вправо +150px | 🔒 Железобетонно | FUN_0040afd1 |
| 120 | Формат "label: value" через append ": " (DAT_004b29ac) | 🔒 Железобетонно | FUN_0040c7d4 |



---

## 40. Формирование описания дефекта — `FUN_004140a1`

```c
void* FormatDefectDescription(void* result, byte* rawPacket) {
    // 1. Код категории дефекта:
    byte defect_category = ReadByte(rawPacket + 0x7E) & 0xFF;   // offset 126
    
    // 2. Тип контроля:
    char control_type = (char)rawPacket[0x48];                   // offset 72
    
    // 3. Формирование шаблона:
    char* template = FUN_00413f6f(control_type, defect_category);
    //   → 10+ шаблонов по коду дефекта (case 1..10)
    //   → case 1 дополнительно учитывает control_type/100
    
    // 4. Если шаблон не пуст → добавляем перенос строки
    if (template != "") {
        result = template + "\n";
    }
}
...
**Ключевые смещения для поля «Дефект»:**
- `raw_buf[0x48]` (72 dec) = тип контроля
- `raw_buf[0x7E]` (126 dec) = код категории дефекта

---

## 41. Callback'и видимости полей (подтверждённые)

| Функция | Проверка | Значение | Смысл |
|---------|----------|----------|-------|
| `FUN_00414088` | `rawPacket[0x106] != 0` | true/false | Поле скорости/диаметра видимо |
| `FUN_0040fedc` | `(rawPacket[400] & 4) != 0` | true/false | Есть B-scan данные |
| `FUN_0040fef4` | `!(rawPacket[400] & 4)` | true/false | Нет B-scan (инверсия) |
| `FUN_00413696` | `rawPacket[0x147]>0x10 && rawPacket[0x106]!=0 && (rawPacket[0x137]&4)` | true/false | Строб с активным флагом |
| `FUN_004136cf` | `rawPacket[0x147]>0x10 && rawPacket[0xAF]!=0` | true/false | Строб + режим |
| `FUN_00413b1a` | `(rawPacket[400] & 4) != 0` | true/false | B-scan present (= FUN_0040fedc) |
| `FUN_00413b4d` | `ReadShort(rawPacket + 0x4B) != 0` | true/false | Доп.поле !=0 |

---

## 42. 7 конфигураций массива полей (по версиям прибора)

| # | DAT адрес массива | Функция инициализации | Предполагаемая версия |
|---|---|---|---|
| 1 | `0x004bd128` | строка ~13773 | Базовая (Толщиномер) |
| 2 | `0x004c12b8` | строка ~14126 | Базовая расширенная |
| 3 | `0x004c5578` | `FUN_0040fc5a` | Базовая (свар.шов) |
| 4 | `0x004ea2a8` | `FUN_0041375a` | Вагонная (ось РУ1) |
| 5 | `0x004efa08` | `FUN_00413b7a` | Вагонная (колёсная пара) |
| 6 | `0x004f4218` | `FUN_004141aa` | Рельсовая (Р50/Р65) |
| 7 | `0x004f5880` | строка ~17673 | Рельсовая (стрелка/стык) |

Все 7 имеют **одинаковые первые 6 полей** (Дата, Номер, Амплитуда, Толщина_усл, Амплитуда_усл, Скорость, Диаметр), но отличаются **полем «Дефект»** (разные callback'и) и **резервом** (количеством нулей после 7-го поля).

---

## 43. Полная сводная карта смещений в пакете (финальная)

| Offset | Dec | Size | Тип | Поле | Как декодируется | Callback/Conditions |
|---|---|---|---|---|---|---|
| +0x00 | 0 | 2 | LE16 | devId | integer | всегда |
| +0x07 | 7 | 3 | date[3] | date | [day,month,year-2000] | всегда |
| +0x0A | 10 | 2 | time[2] | time | [hours,minutes] | всегда |
| +0x0C | 12 | 1 | flag | defect_flag | 0=нет дефекта | A-scan/composite |
| +0x10 | 16 | 2 | LE16 | blockAddr | TLV tag | всегда |
| +0x11 | 17 | 11 | LUT | passport_primary | reverse-LUT decode | A-scan/composite |
| +0x14 | 20 | 2 | LE16 | TypeVar (tcode) | integer | всегда |
| +0x20 | 32 | 2 | LE16 | category_code | /1000→category | всегда |
| +0x21 | 33 | 7 | LUT | passport_secondary | reverse-LUT decode | A-scan |
| +0x27 | 39 | 5 | mixed | date2_raw | packed BCD/date? | настройки |
| +0x35 | 53 | 2 | LE16 | thickness | мм×10 | generic |
| +0x37 | 55 | 2 | LE16 | velocity | м/с | generic |
| +0x38 | 56 | 2 | LE16 | amplitude_dB | дБ×2 | A-scan |
| +0x39 | 57 | 2 | LE16 | depth_mm | мм | TypeVar-specific |
| +0x3A | 58 | 2 | LE16 | delay_us | мкс | A-scan |
| +0x3C | 60 | 2 | LE16 | gate_position | отсчёты | composite |
| +0x3E | 62 | 2 | LE16 | echo_count | шт | composite |
| +0x45 | 69 | 2 | LE16 | zone_width | отсчёты | A-scan |
| +0x48 | 72 | 1 | byte | control_type | char→FUN_00413f6f | отчёт |
| +0x4B | 75 | 2 | LE16 | extra_flag | !=0 → доп.поля | видимость |
| +0x5B | 91 | 2 | LE16 | thickness_field | тип 0x25 "ifLength" | условное |
| +0x5D | 93 | 2 | LE16 | amplitude_field | тип 0x25 "ifHight" | условное |
| +0x5E | 94 | 2 | LE16 | total_length | мм | generic |
| +0x7E | 126 | 2 | LE16 | channel_code | /100→канал, &0xFF→дефект | отчёт/DGS |
| +0xAF | 175 | 1 | byte | mode_flag | !=0 → режим | видимость |
| +0x101 | 257 | 1 | byte | extended_flags | &2 → расширенный DGS | режим |
| +0x106 | 262 | 1 | byte | speed_visible | !=0 → поле скорости | FUN_00414088 |
| +0x137 | 311 | 1 | byte | gate_flags | бит 2 → строб активен | FUN_00413696 |
| +0x147 | 327 | 1 | byte | gate_threshold | >0x10 → строб есть | видимость |
| +0x15D | 349 | 2 | LE16 | record_number | номер записи | всегда |
| +0x160 | 352 | 6 | Real48 | speed_of_sound | мм/мкс (Delphi float) | тип 0x42/0x43 |
| +0x190 | 400 | 1 | byte | mode_byte | бит0+биты1-2→режим | FUN_00411799 |

---

## 44. Сводка (раунд 9 — финальная проверка)

| # | Находка | Статус | Источник |
|---|---------|--------|----------|
| 111 | **Шейка** = тип 7 (0=Открытая, 1=С кольцами, 2=С буксой) | 🔒 Железобетонно | case 7 DGS decoder |
| 112 | Преобразователь = тип 0x0E (4 варианта) | 🔒 Железобетонно | case 0x0E |
| 113-120 | HTML/WordML генераторы, stride, категории | 🔒 Железобетонно | FUN_0040c7d4/FUN_0040d02b |
| 121 | Описание дефекта: raw[0x48]=control_type + raw[0x7E]=defect_code → шаблон | 🔒 Железобетонно | FUN_004140a1 |
| 122 | Видимость скорости: raw[0x106] != 0 | 🔒 Железобетонно | FUN_00414088 |
| 123 | B-scan flag: raw[400] & 4 | 🔒 Железобетонно | FUN_0040fedc |
| 124 | 7 конфигураций массива полей (3 версии × 2 + 1) | 🔒 Железобетонно | 7× DAT_004c0064 |
| 125 | Все 7 конфигураций имеют одинаковые первые 6 полей | 🔒 Железобетонно | сравнение init-функций |
| 126 | Номер записи: LE16 @ offset +0x15D (349 dec) — подтверждено во всех 7 конфигурациях | 🔒 Железобетонно | все init-функции |
| 127 | Скорость звука: Real48 @ offset +0x160 (352 dec) — подтверждено во всех 7 | 🔒 Железобетонно | все init-функции |
| 128 | FUN_0040fc36 и FUN_00414074 возвращают 0x2B6 (694) — длина отчёта? | 🔒 Железобетонно | прямое чтение |
| 129 | FUN_0040fc40 и FUN_0041407e возвращают 0xFD6 (4054) — длина расширенного? | 🔒 Железобетонно | прямое чтение |
| 130 | Полная карта 30+ смещений в сыром пакете (см. §43) | 🔒 Железобетонно | множество FUN |
